...
# The GUI repo is cloned on demand into the shared .tools/gui-src next to the
# Kristal engine — not a submodule.
# zh_hans: 开发者模式：源码运行 GUI（需要 Rust + Node）。源码会按需 clone 到共享 .tools/gui-src（Kristal 引擎旁）。
gui-dev:
